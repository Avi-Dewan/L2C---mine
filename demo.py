...
def train(epoch, train_loader, learner, args):
    ...
    for i, (input, target) in enumerate(train_loader):
        ...
        if args.use_gpu:
            input = input.cuda()
            target = target.cuda()  # async+pin_mem = minor speed gain
...
def evaluate(eval_loader, model, args):
    ...
    if args.loss == 'CE':
        ...
        confusion.show(width=15, row_labels=['GT_dis-simi', 'GT_simi'], column_labels=['Pred_dis-simi', 'Pred_simi'])
        print("Confusion sns show")
        confusion.show_sns(args.dataset, fig_width=10, fig_height=8, row_labels=['GT_dis-simi','GT_simi'],column_labels=['Pred_dis-simi','Pred_simi'])
